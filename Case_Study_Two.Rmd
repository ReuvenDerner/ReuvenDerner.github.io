---
title: "Case_Study_2"
author: "Robert Derner"
date: "12/7/2021"
output: html_document
---

[Link](https://smu.zoom.us/rec/play/n4I_gxBul9x6l0eqhvpHsmFpAwzIzmpcFZ3wRvy7FUutJ_6SSmRVyWtLGUXYXvzTOOIcFhxcNsnJwnIH.r2k03etIa08uOa87) to recorded presentation.
...
Read the data
```{r message=FALSE, warning=FALSE}
attr <- read_csv('https://raw.githubusercontent.com/ReuvenDerner/CaseStudy2DDS/main/CaseStudy2.csv', show_col_types=F)
```
...
Accuracy = 86% Sensi = 61.42% Speci = 90.82%
```{r}
set.seed(80)
...
May have to rerun a few times, average sensi was 60% low of 45 high of 75
Accuracy = 87.61% Sensi = 59.45% Speci = 93.37%
```{r}
...
train.attr <- read_csv('https://raw.githubusercontent.com/ReuvenDerner/CaseStudy2DDS/main/CaseStudy2CompSet%20No%20Attrition.csv', show_col_types=F)
```
...
head(p.attr.df)
write.csv(p.attr.df,"C:\\Users\\Robert.Derner\\Desktop\\Case2PredictionsLamDernerAttrition.csv", row.names = FALSE)
```
...
Model Diagnostics
```{r}
ols_plot_diagnostics(fit.sal) # all diagnostic plots
ols_plot_cooksd_chart(fit.sal) # cooks d
ols_plot_comp_plus_resid(fit.sal)
ols_plot_obs_fit(fit.sal) # actual vs fitted values
ols_plot_resid_fit(fit.sal) # residual vs fitted values
ols_plot_added_variable(fit.sal) # partial residuals / added variable
...
Read the train data
```{r message=FALSE, warning=FALSE, eval=FALSE}
train.salary <- read_csv('https://raw.githubusercontent.com/ReuvenDerner/CaseStudy2DDS/main/CaseStudy2CompSetNoSalary.csv', show_col_types = F)
```
...
head(p.sal.df)
write.csv(p.sal.df,"C:\\Users\\Robert.Derner\\Desktop\\Case2PredictionsLamDernerSalary.csv", row.names = FALSE)
```